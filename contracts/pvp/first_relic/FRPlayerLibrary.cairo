...
func FirstRelicCombat_get_players{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
...
func FirstRelicCombat_get_koma{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
...
func FirstRelicCombat_get_komas{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
...
func FirstRelicCombat_init_player{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    combat_id: felt, account: felt
) -> (next_seed: felt) {
    ...
    let koma = Koma(
        ...
        status=KOMA_STATUS_STATIC,
        health=100,
        max_health=100,
        agility=7,
        move_speed=KOMA_MOVING_SPEED,
        props_weight=0,
        props_max_weight=1000,
        workers_count=3,
        ...
        ore_amount=10000000,
        atk=KOMA_ATK,
        defense=KOMA_DEFENSE,
        worker_mining_speed=WORKER_MINING_SPEED,
    );
...
func FirstRelicCombat_move{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    combat_id: felt, account: felt, to: Coordinate
) {
    alloc_locals;
    let (koma) = FirstRelicCombat_komas.read(combat_id, account);
    let (actual_status, actual_at) = FirstRelicCombat_get_koma_actual_coordinate(
        combat_id, account, koma
    );
    let (block_timestamp) = get_block_timestamp();
    ...
    let movement = Movment(
        new_koma.coordinate, to, block_timestamp, block_timestamp + time_need + 1
    );
...
func _get_komas{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    combat_id: felt, accounts_len: felt, accounts: felt*, data_len: felt, data: Koma*
) -> (data_len: felt, data: Koma*) {
    if (accounts_len == 0) {
        return (data_len, data);
    }
...
func _get_komas_movments{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    combat_id: felt, accounts_len: felt, accounts: felt*, data_len: felt, data: Movment*
) -> (data_len: felt, data: Movment*) {
    if (accounts_len == 0) {
        return (data_len, data);
    }
...
func _get_distance{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    from_: Coordinate, to: Coordinate
) -> (distance: felt) {
