...
namespace OreLibrary {
    func mine_ore{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (empty_time) = _get_ore_empty_timestamp(
            ore.current_supply, mining_speed, block_timestamp
        );
        let new_ore = Ore(
            coordinate=ore.coordinate,
            total_supply=ore.total_supply,
            current_supply=ore.current_supply,
            collectable_supply=ore.collectable_supply,
            mining_account=account,
            mining_workers_count=mining_workers_count,
            mining_speed=mining_speed,
            structure_hp=ORE_STRUCTURE_HP_PER_WORKER * workers_count,
            structure_max_hp=ORE_STRUCTURE_HP_PER_WORKER * workers_count,
        ...
        let new_koma = Koma(
            ...
            koma.workers_count,
            koma.mining_workers_count + workers_count,
            koma.drones_count,
            koma.action_radius,
            koma.element,
            koma.ore_amount,
    ...
    func recall_workers{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        combat_id: felt, account: felt, target: Coordinate, workers_count: felt
    ) {
        alloc_locals;
        ...
        let new_ore = Ore(
            ...
            structure_max_hp=structure_max_hp,
            start_time=block_timestamp,
            empty_time=empty_time,
        );
        let new_koma = Koma(
    ...
    func produce_bot{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    func collect_ore{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let ore_updated = Ore(
            ...
            mining_account=ore.mining_account,
            mining_workers_count=ore.mining_workers_count,
            mining_speed=ore.mining_speed,
            structure_hp=ore.structure_hp,
            structure_max_hp=ore.structure_max_hp,
            start_time=ore.start_time,
            empty_time=ore.empty_time,
        );
        let koma_updated = Koma(
            account=koma.account,
            coordinate=koma.coordinate,
            status=koma.status,
            health=koma.health,
            max_health=koma.max_health,
            agility=koma.agility,
            move_speed=koma.move_speed,
            props_weight=koma.props_weight,
            props_max_weight=koma.props_max_weight,
            workers_count=koma.workers_count,
            mining_workers_count=koma.mining_workers_count,
            drones_count=koma.drones_count,
            action_radius=koma.action_radius,
            element=koma.element,
            ore_amount=koma.ore_amount + ore.collectable_supply,
            atk=koma.atk,
            defense=koma.defense,
            worker_mining_speed=koma.worker_mining_speed,
        );
    ...
    func attack_ore{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        combat_id: felt, account: felt, target: Coordinate
    ) {
        alloc_locals;
        ...
        if (destroyed == TRUE) {
            ...
            let koma_miner_updated = Koma(
                account=koma_miner.account,
                coordinate=koma_miner.coordinate,
                status=koma_miner.status,
                health=koma_miner.health,
                max_health=koma_miner.max_health,
                agility=koma_miner.agility,
                move_speed=koma_miner.move_speed,
                props_weight=koma_miner.props_weight,
                props_max_weight=koma_miner.props_max_weight,
                workers_count=koma_miner.workers_count - ore.mining_workers_count,
                mining_workers_count=koma_miner.mining_workers_count - ore.mining_workers_count,
                drones_count=koma_miner.drones_count,
                action_radius=koma_miner.action_radius,
                element=koma_miner.element,
                ore_amount=koma_miner.ore_amount,
                atk=koma_miner.atk,
                defense=koma_miner.defense,
                worker_mining_speed=koma_miner.worker_mining_speed,
            );
    ...
    func get_ore_count{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        combat_id: felt
    ) -> (count: felt) {
    ...
    func get_ores{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    func get_ore_by_coordinate{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    func clear_koma_ores{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    func update_ore{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        let (mined_amount) = min(
            ore.current_supply, (end_time - ore.start_time) * ore.mining_speed
        );
        ...
        let new_ore = Ore(
            ...
            collectable_supply=collectable_supply,
            mining_account=ore.mining_account,
            mining_workers_count=ore.mining_workers_count,
            mining_speed=ore.mining_speed,
            structure_hp=ore.structure_hp,
...
func _can_mine{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    combat_id: felt, account: felt
) -> (res: felt) {
...
func _get_ore_empty_timestamp{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    current_supply: felt, mining_speed: felt, start_time: felt
) -> (empty_time: felt) {
    if (current_supply == 0) {
        return (0,);
    }
...
func _clear_koma_ores{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (ore_coordinate) = FirstRelicCombat_koma_ore_coordinates_by_index.read(
...
func _remove_koma_ore_from_list{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    combat_id: felt, account: felt, target: Coordinate, index: felt, len: felt
) -> (removed: felt) {
    if (index == len) {
        return (FALSE,);
    }
    let (coordinate_by_index) = FirstRelicCombat_koma_ore_coordinates_by_index.read(
        combat_id, account, index
    );
    if (target.x - coordinate_by_index.x + target.y - coordinate_by_index.y == 0) {
        ...
        if (index != len - 1) {
            let (last_ore_coordinate) = FirstRelicCombat_koma_ore_coordinates_by_index.read(
                combat_id, account, len - 1
            );
            FirstRelicCombat_koma_ore_coordinates_by_index.write(
                combat_id, account, index, last_ore_coordinate
            );
            tempvar syscall_ptr = syscall_ptr;
            tempvar pedersen_ptr = pedersen_ptr;
            tempvar range_check_ptr = range_check_ptr;
...
func _get_produce_bot_required_ore_amount{range_check_ptr}(
    quantity_now: felt, quantity_produce: felt, ore_amount: felt
) -> (ore_amount: felt) {
    ...
    return _get_produce_bot_required_ore_amount(
        quantity_now + 1, quantity_produce - 1, ore_amount + ore_required
    );
}

// recursively get ore struct array
func _get_ores{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    combat_id: felt, index: felt, length: felt, data_len: felt, data: Ore*
) -> (data_len: felt, data: Ore*) {
    if (length == 0) {
        return (data_len, data);
    }
