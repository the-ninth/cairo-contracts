...
func get_random_number{pedersen_ptr: HashBuiltin*, range_check_ptr}(
    seed: felt, step: felt, mod: felt
) -> (res: felt) {
...
func get_random_number_and_seed{pedersen_ptr: HashBuiltin*, range_check_ptr}(
    seed: felt, mod: felt
) -> (res: felt, next_seed: felt) {
    let hash_ptr = pedersen_ptr;
    with hash_ptr {
        let (hash_state_ptr) = hash_init();
        let (hash_state_ptr) = hash_update_single(hash_state_ptr, seed);
