...
FRBoss_CONTRACT_FILE = contract_path('contracts/pvp/first_relic/third_stage/FR3rdBoss.cairo')
...
async def test_market():
    ...
    # set market config
    await signer.send_transaction(
        owner_contract, frboss_contract.contract_address, 'addBossMeta', [1000,100,100]
    )
    await signer.send_transaction(
...
#     await signer.send_transaction(heros[3], frboss_contract.contract_address, 'join', [])
    ...
    for j in range(4):
        for i in range(9):
            print('action',i)
            await signer.send_transaction(
                heros[i], frboss_contract.contract_address, 'action', [0, j, i+1 ,2,0]
            )
